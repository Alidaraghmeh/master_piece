		...
		<div class="content">
			<div class="viewContent">
					...
					<ul>
						...
						<li><a href='RequestsO.php'>
							...
							if ($eventNotifications != false) {
								echo "<span class='notification-number'>$eventNotifications</span>";
							}
							?>
						</a></li>
						<li class="active"><a href='NewsLettersO.php'>
							<span>NewsLetters</span>
							<?php
							if ($newsletterNotifications != false) {
								echo "<span class='notification-number'>$newsletterNotifications</span>";
							}
							?>
						</a></li>
				...
				<div class="viewCon">
					...
					include("dbCon.php");
					$OS = "SELECT orphanStatus FROM newsletters WHERE `orphanStatus` NOT LIKE '%$idOrph%'";
					$OldS = mysqli_query($db, $OS);
					while ($OldStatus = mysqli_fetch_array($OldS)) {
						$newStatus = $idOrph." - ".$OldStatus[0];
						$UP = "UPDATE `newsletters` SET `orphanStatus`='$newStatus' WHERE `orphanStatus` NOT LIKE '%$idOrph%'";
						mysqli_query($db, $UP);
					}
